...
export default function WebRTCWebSocketPage(): React.ReactNode {
  ...
  const handleCreateRoom = () => {
    ...
    router.push(`/${locale}/web-rtc/websocket/room/${newRoomId}`);
  };

  const handleJoinRoom = () => {
    if (!disabledJoinLink) {
      router.push(`/${locale}/web-rtc/websocket/room/${roomId}`);
    }
  };
  ...
  return (
    <section className="web_rtc_room_page">
      ...
      <Alert severity="warning" sx={{ mb: 3 }}>
        注意：WebSocket 在 serverless
        環境可能無法正常運作。建議在本地開發環境測試。
      </Alert>
