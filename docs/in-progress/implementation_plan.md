...
> 3. **Socket.IO**: ❌ **不實作** - Next.js 不支援內建 WebSocket 伺服器，僅保留客戶端可連接外部伺服器
> 4. **WebSocket Server**: ❌ **不實作** - Next.js 不支援內建 WebSocket 伺服器，僅保留客戶端可連接外部伺服器
...
> - 📝 如需 Socket.IO/WebSocket: 請部署獨立伺服器 (Railway/Render 等)
...
- [ ] Face Swap API
- [ ] Nuxt Server APIs 轉換
- ❌ Socket.IO server routes (Next.js 不支援，僅保留客戶端)
- ✅ SSE server routes (已完成)
- ✅ WebRTC signaling routes (已完成)
