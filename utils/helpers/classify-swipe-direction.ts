...
export type swipeDirectionOutput = swipeDirection | null;
interface options {
 ...
 * 判斷觸摸手勢是垂直還是水平。
 *
 * @param startPoint - 滑動開始時的座標點，包含 clientX 和 clientY。
 * @param endPoint - 滑動結束時的座標點，包含 clientX 和 clientY。
 * @param [options] - 可選的配置選項。
 * @param [options.minSwipeDistance=30] - 判斷為有效滑動所需的最小距離（像素）,預設為30。
 * @param [options.angleThreshold=45] - 判斷為垂直或水平滑動的最大角度偏差（度）,預設為45。
