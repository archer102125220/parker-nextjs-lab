...
- **DB Modification Confirmation:** Before ANY schema change, ask "Is this project deployed to production?" → Not deployed = modify + `yarn initDB`; Deployed = create NEW migrations
- **Migration Policy:** Early dev = modify original migrations directly (not addColumn) + `yarn initDB`; Post-prod = create new files
